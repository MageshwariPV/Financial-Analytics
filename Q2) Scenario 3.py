...
#Finding the mean return and covariance matrix 
returns_2 = data_split2.mean()
cov_matrix_2 = data_split2.cov()

assets = data_split2.columns
...
def benchmark_constraint(x):
    weights = x[:5]
    return np.sum(returns_2 * weights) - np.sum(returns_2 * benchmark_weights)
...
cons = [return_con_sub2, risk_con_sub2, max_weight_con, benchmark_con]
...
for alpha in alpha_values:
    cons[3]['fun'] = lambda x: np.sum(returns_2 * x[:5]) - np.sum(returns_2 * benchmark_weights) + alpha * (np.sum(abs(x[:5] - current_weights) * cost) - solution.fun)
    solution = minimize(objective_function, x0,
                         method = "SLSQP",
                         bounds = bounds,
                         constraints = cons)
